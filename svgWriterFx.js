    ...
    function SVGWriterFx() {
        ...
        var glowHelperFunction = function (ctx, glow, glowType, ind) {
            ...
            if (glow.gradient) {
                ...
                // Gradient map.
                nSegs = self.findMatchingDistributedNSegs(gradient.stops);
                colors = self.calcDistributedColors(gradient.stops, nSegs);
                self.createGradientMap(ctx, colors);
            } else {
        ...
        // FIXME: Move Gradient map code to separate file.
